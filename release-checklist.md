...
environment**. Deploy steps: `RENDER.md`.
...
## Resume here
1. **Ship the current code to `main`**: merge `develop` → `main`. Render auto-deploys `main`.
2. Work top-to-bottom through **Ops** — each section needs your Google Cloud / Supabase / Resend / Render dashboards.
...
- **Topology:** **one** environment — the existing hosted stack **is** production (one Render
  service, one Supabase project). Local Supabase covers dev + migrations. No staging.
- **Deploy:** Render auto-deploys `main`; all dev is local; merge to `main` to release.
...
The auth posture is **env-flag driven** (`apps/web/src/shared/lib/auth-config.ts` → `getAuthPolicy`): Magic
...
### Render + domain
- [ ] Set the service's **deploy branch to `main`** and leave **Root Directory** empty.
...
- [ ] Migrations run as a **pre-deploy step**, never inside `pnpm start` (see `RENDER.md`).
- [ ] DNS: point `sheetless.fitness` at the Render service and redirect non-canonical hosts (Render provides TLS — also satisfies PWA HTTPS).
...
- [ ] **Error visibility:** know where to read Render logs + Supabase logs; consider a lightweight
...
Set `AUTH_ALLOWLIST_ENABLED=true` in Render, turn "Allow new users to sign up" **off** in Supabase,
