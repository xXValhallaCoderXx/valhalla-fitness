...
import { getPinnedTemplateDefinition } from './template-data'
...
export async function getActiveProgram(ctx: UserContext): Promise<ProgramInstance | null> {
...
export async function updateProgramCurrentWeekIndex(
  ...
  const refreshed = await getActiveProgram(ctx)
  if (
...
export async function resolveProgressionDecision(
  ...
  return getPendingDecisions(ctx, decision.program_instance_id ?? undefined)
}
