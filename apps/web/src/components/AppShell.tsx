...
const navItems = [
  ...
  { to: '/templates', label: 'Programmes', icon: Layers3 },
] as const
...
export function AppShell({ user, children }: { user: AuthUser | null; children: ReactNode }) {
  ...
  return (
    <Box
      ...
      className="vf-app-shell flex overflow-hidden"
      data-testid="app-shell"
    >
      <AppSidebar items={navItems} activeSection={navigation.activeBottomNavSection} />
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        >
          <div className="vf-app-header-inner mx-auto grid h-12 max-w-[1180px] grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3">
            <div className={cn(!navigation.backTarget && 'lg:hidden')}>
              <AppHeaderLeading backTarget={navigation.backTarget} />
            </div>
            <Box
              component="nav"
              className="hidden items-center justify-center gap-1 rounded-lg p-1 md:flex lg:hidden"
              bg="var(--vf-surface-2)"
