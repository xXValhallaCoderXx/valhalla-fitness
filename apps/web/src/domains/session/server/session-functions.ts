...
import { ServiceError } from '~/shared/server/service-error'
import {
...
export async function getTodayInternal(
  context?: AuthenticatedServiceContext,
): Promise<TodayPayload> {
  const resolvedContext = await requireUser(context)
  const { supabase, user } = resolvedContext
  let activeProgram = await getActiveProgramInternal(resolvedContext)
  ...
  const activeSession = activeSessionRow
    ? await getSessionInternal(activeSessionRow.id, resolvedContext)
    : null
  ...
  const completedSession = completedSessionRow
    ? await getSessionInternal(completedSessionRow.id, resolvedContext)
    : null
  if (!activeSession) {
  ...
  const pendingDecisions = await getPendingDecisionsInternal(activeProgram.id, resolvedContext)
...
export async function getSessionInternal(
  ...
  const { data: sessionRow, error } = await supabase
    ...
    .maybeSingle()
  if (error) throw new Error(error.message)
  if (!sessionRow) throw new ServiceError('NOT_FOUND', 'Workout session not found.', 404)
...
export const getTodayFn = createServerFn({ method: 'GET' }).handler(async () => getTodayInternal())
...
export async function startPlannedSession(
  ...
  if (existing) {
    await insertExerciseLogsForMovements(
      supabase,
      user.id,
      existing.id,
      (existing.prescription_snapshot as PlannedSession).movements,
    )
    return getSessionInternal(existing.id, context)
  }

  const today = await getTodayInternal(context)
  if (!today.activeProgram || !today.plannedSession) {
    throw new ServiceError('NO_PLANNED_SESSION', 'No planned session', 409)
  }
  if (today.activeSession) return today.activeSession
  if (today.pendingDecisions.length > 0) {
    throw new ServiceError(
      'PENDING_PROGRESSION',
      'Resolve your pending progression changes before starting the next session.',
      409,
    )
  }
    ...
    if (error.code === '23505') {
      ...
      if (duplicate) {
        await insertExerciseLogsForMovements(
          ...
          user.id,
          duplicate.id,
          (duplicate.prescription_snapshot as PlannedSession).movements,
        )
        return getSessionInternal(duplicate.id, context)
      }
    }
  ...
  return getSessionInternal(session.id, context)
}
...
export const startSessionFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    return startPlannedSession(context, data)
  })
  ...
  for (const movement of movements) {
    ...
    const { data: insertedExercise, error: exerciseError } = await supabase
      ...
      .upsert(
        ...
        { onConflict: 'session_id,slot_id', ignoreDuplicates: true },
      )
      .select('*')
      .maybeSingle()
    if (exerciseError) throw new Error(exerciseError.message)
    let exercise = insertedExercise
    if (!exercise) {
      const { data: existingExercise, error: existingExerciseError } = await supabase
        ...
        .eq('slot_id', slotId)
        .single()
      if (existingExerciseError) throw new Error(existingExerciseError.message)
      exercise = existingExercise
    }
    const { error: setError } = await supabase.from('set_logs').upsert(
      ...
      { onConflict: 'exercise_log_id,set_index', ignoreDuplicates: true },
    )
    if (setError) throw new Error(setError.message)
  }
}
...
export type UpdateSetLogServiceInput = {
  sessionId: string
...
export async function updateSetLog(
  ...
  if (sessionRow.status === 'completed') {
    throw new ServiceError(
      ...
      'This session is already finished. Completed sessions cannot be edited.',
      409,
    )
  }
  if (sessionRow.status !== 'in_progress') {
    throw new ServiceError('SESSION_NOT_EDITABLE', 'Only in-progress sessions can be edited.', 409)
  }
  ...
  const { data: exercise, error: exerciseError } = await supabase
    ...
    .maybeSingle()
  if (exerciseError) throw new Error(exerciseError.message)
  if (!exercise) throw new ServiceError('NOT_FOUND', 'Exercise log not found.', 404)
  ...
  const { data: updatedSet, error } = await supabase
    ...
    .maybeSingle()
  if (error) throw new Error(error.message)
  if (!updatedSet) throw new ServiceError('NOT_FOUND', 'Set log not found.', 404)
  return getSessionInternal(data.sessionId, context)
}

export const upsertSetLogFn = createServerFn({ method: 'POST' })
  .validator(
    (data: UpdateSetLogServiceInput) => data,
  )
  .handler(async ({ data }) => {
    const context = await requireUser()
    return updateSetLog(context, data)
  })
