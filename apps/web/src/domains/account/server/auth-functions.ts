...
import type { MagicLinkIntent } from '@sheetless/api'
...
export async function getMagicLinkIntent(emailInput: string): Promise<MagicLinkIntent> {
  const config = await getSupabasePublicConfig()
  if (!config) throw new Error('Supabase is not configured for magic links.')
