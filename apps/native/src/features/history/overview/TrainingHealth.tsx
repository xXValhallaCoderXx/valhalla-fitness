...
export function TrainingHealth({ insights, gating }: { insights: HistoryInsights; gating: InsightGating }) {
  ...
  return <>
    <Panel style={{ gap: spacing.sm, padding: spacing.md }}>
      <SectionLabel>{insightCardLabel('consistency', mode)} · available workouts</SectionLabel>
    ...
    <Panel style={{ gap: spacing.sm, padding: spacing.md }}>
      <SectionLabel>{insightCardLabel('effort', mode)} · last 6 weeks</SectionLabel>
      <Badge tone={calibration.signal === 'leaning_hard' ? 'warning' : 'neutral'}>{calibrationSignalLabels[calibration.signal]}</Badge>
      <Text size="sm">{calibration.meanGap === null ? 'Log how many reps you had left to compare effort with the plan.'
        : !isFull ? 'This compares your logged reps left with the effort prescribed in your plan.'
