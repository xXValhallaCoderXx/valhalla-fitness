...
- Follow `AGENTS.md` for the complete machine-facing repository rules. Use `README.md` as the sole
  human-facing source for product scope, release state, architecture, the DSL, testing, and
  deployment.
- Treat this as a TanStack Start/Vite + React 19 + TypeScript app using TanStack Router, React Query, Supabase, Mantine, layout-only Tailwind CSS 4, Zod, and pnpm.
...
- Keep domain UI, server functions, pure logic, and query options in `src/domains/{account,program,session,history,movement,onboarding}/`. Keep reusable UI in `src/components/atoms` and `src/components/molecules`. Promote cross-domain helpers to `src/shared/*` only when 3+ domains need them.
...
- Workout saving is online-only. Keep optimistic state in the React Query session cache, retain
  explicit saving/failure/retry UI, and do not add a durable offline database or replay queue
  without an explicit product/architecture decision recorded in `README.md`.
