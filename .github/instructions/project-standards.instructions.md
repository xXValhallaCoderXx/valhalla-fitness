...
- Keep shared reusable UI in `src/components/`, feature-scoped UI in `src/features/{feature}/components/`, shared app/business logic in `src/lib/`, server/Supabase helpers in `src/server/`, shared domain types in `src/types/`, and global styling in `src/styles/`.
...
- For UI, prefer Mantine-backed local wrappers. Shared atoms live in `src/components/atoms/`, shared molecules live in `src/components/molecules/`, and larger stateful/domain UI belongs under `src/features/{feature}/components/`. `src/components/ui.tsx` and `src/components/workout.tsx` are compatibility shims for migration.
- Tailwind is still allowed as a transition/layout utility. Compose class names with `cn()` when needed and preserve existing CSS variables such as `--bg`, `--surface`, `--border`, `--text`, `--muted`, `--action`, `--success`, `--warning`, and `--danger`.
